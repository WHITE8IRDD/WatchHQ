...
const Movies: React.FC = () => {
  ...
  return (
    <div className="flex flex-col h-full max-w-[1400px] mx-auto px-8 py-6 w-full">
      ...
      <div className="flex items-center gap-3 mb-4 flex-shrink-0">
        ...
        {syncStatus === 'syncing' && (
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-accent">Syncing movies...</span>
          </div>
        )}
        {syncStatus === 'done' && items.length === 0 && (
          <span className="text-xs text-text-tertiary">Synced — no movies found in this account</span>
        )}
        {syncStatus === 'idle' && items.length === 0 && !syncing && !loading && (
          <span className="text-xs text-text-tertiary">Not yet synced</span>
        )}
      </div>
      ...
      >
        ...
        <div className="flex gap-3 px-6 pb-6">
          <button
            ...
            className="flex-1 flex items-center justify-center gap-2 py-3 bg-white text-black rounded-xl font-medium hover:bg-white/90 transition-all"
          >
            <Play size={18} /> Play
          </button>
          <button
